...
public class MilkEntryDeduplicator {
    ...
    private void ensureUniqueIndex() {
        try {
            mongoTemplate.indexOps(MilkEntry.class).ensureIndex(
                    new Index()
                            .on("customerId", Sort.Direction.ASC)
                            .on("date", Sort.Direction.ASC)
                            .unique()
                            .named("customer_date_unique_idx")
            );
            log.debug("Unique index customer_date_unique_idx ensured on milk_entries");
        } catch (Exception e) {
            log.warn("Could not create unique index on milk_entries: {}", e.getMessage());
        }
    }
