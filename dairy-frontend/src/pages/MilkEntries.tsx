...
export default function MilkEntries() {
  ...
  const [autoGenStartDate, setAutoGenStartDate] = useState(date)
  const [autoGenEndDate, setAutoGenEndDate] = useState(date)
  const [autoGenIsRange, setAutoGenIsRange] = useState(false)
  ...
  useEffect(() => {
    ...
    setAutoGenStartDate(date)
    setAutoGenEndDate(date)
  }, [date])
  ...
  const confirmAutoGenerate = async () => {
    try {
      ...
      let res;
      if (autoGenIsRange) {
        res = await axios.post(`/api/milk-entries/auto-generate-range?startDate=${autoGenStartDate}&endDate=${autoGenEndDate}`)
      } else {
        res = await axios.post(`/api/milk-entries/auto-generate?date=${autoGenStartDate}`)
      }
      ...
      if (res.data.success) {
        ...
        if (count > 0) {
          ...
          void fetchCustomers()
          // refresh if current view date is affected
          if (date >= autoGenStartDate && date <= autoGenEndDate) {
            void fetchEntries(date, { showLoading: false })
          } else if (!autoGenIsRange && date === autoGenStartDate) {
            void fetchEntries(date, { showLoading: false })
          }
          setIsAutoGenerateDialogOpen(false);
        } else {
          setAutoGenError(null)
          toast.success(language === 'mr' ? 'नवीन नोंदीची गरज नव्हती.' : 'No new entries were needed.')
          setIsAutoGenerateDialogOpen(false)
        }
      }
    } catch (error) {
      ...
      setAutoGenError(language === 'mr' ? `ऑटो जनरेट करण्यात त्रुटी: ${errorMsg}` : `Failed to auto-generate entries: ${errorMsg}`)
    } finally {
        ...
        <div className="flex w-full min-w-0 flex-col gap-3 xl:w-auto xl:items-end">
          ...
          <div className="flex w-full min-w-0 flex-wrap items-center gap-2 xl:w-auto xl:justify-end">
            <Button 
              ...
              className="h-auto min-w-0 flex-1 gap-2 rounded-[1.25rem] border-emerald-200 bg-emerald-50 px-4 py-2 text-emerald-700 hover:bg-emerald-100 dark:border-zinc-700 dark:bg-[#111111] dark:text-white dark:hover:bg-black xl:flex-none"
            >
            ...
            <Dialog open={isOverrideDialogOpen} onOpenChange={(open) => { setIsOverrideDialogOpen(open); if(open) setOverrideError(''); }}>
              <DialogTrigger asChild>
                <Button variant="outline" className="h-auto min-w-0 flex-1 gap-2 rounded-[1.25rem] border-blue-200 bg-blue-50 px-4 py-2 text-blue-700 hover:bg-blue-100 dark:border-zinc-700 dark:bg-[#111111] dark:text-white dark:hover:bg-black xl:flex-none">
            ...
            <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if(open) setNoDeliveryError(''); }}>
              <DialogTrigger asChild>
              <Button variant="outline" className="h-auto min-w-0 flex-1 gap-2 rounded-[1.25rem] border-rose-200 bg-rose-50 px-4 py-2 text-rose-700 hover:bg-rose-100 dark:border-zinc-700 dark:bg-[#111111] dark:text-white dark:hover:bg-black xl:flex-none">
          ...
          <div className="flex w-full min-w-0 max-w-full items-center justify-between gap-1 rounded-[1.25rem] border border-white/80 bg-white/60 p-1.5 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-[#111111] xl:w-auto xl:min-w-[210px] xl:justify-start">
            <button onClick={handlePrevDay} className="p-1.5 hover:bg-white rounded-xl transition-colors text-slate-500 hover:text-slate-800 dark:hover:bg-zinc-800 dark:text-slate-300 dark:hover:text-white">
            ...
            <div className="flex min-w-0 flex-1 items-center justify-center px-1 xl:flex-initial">
              <input 
                ...
                className="w-full min-w-0 max-w-[150px] cursor-pointer border-none bg-transparent text-center font-medium text-slate-700 focus:outline-none dark:text-white xl:max-w-[135px]"
              />
            </div>
            <button onClick={handleNextDay} className="p-1.5 hover:bg-white rounded-xl transition-colors text-slate-500 hover:text-slate-800 dark:hover:bg-zinc-800 dark:text-slate-300 dark:hover:text-white">
      ...
      <div className="bg-white/40 dark:bg-[#101010] backdrop-blur-xl rounded-[2rem] border border-white/60 dark:border-zinc-700/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-6 min-w-0">
            ...
            <div className="flex gap-4 self-start sm:self-auto">
              <div className="text-right">
                ...
                <p className="text-lg font-bold text-blue-600 dark:text-white">{totalLiters.toFixed(1)} L</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">{t(language, 'totalValue')}</p>
                <p className="text-lg font-bold text-emerald-600 dark:text-white">₹{totalAmount.toFixed(2)}</p>
              </div>
            </div>
          </div>
          ...
          {totalOverridesCount > 0 && (
            <div className="mb-6">
              ...
              {isOverridesExpanded && (
                <div className="mt-3 grid grid-cols-1 gap-4 rounded-[1.8rem] border border-slate-100 bg-slate-50/60 p-3 backdrop-blur-sm dark:border-zinc-700 dark:bg-black/40">
                  ...
                  {activeSpecialConditions.length > 0 && (
                    <div className="rounded-[1.6rem] border border-blue-100 bg-blue-50/70 p-3 dark:border-zinc-700 dark:bg-[#141414]">
                      <button
                        type="button"
                        onClick={() => setIsSpecialQuantitiesExpanded(!isSpecialQuantitiesExpanded)}
                        className="flex w-full items-center justify-between gap-3 text-left"
                      >
                        <div className="flex min-w-0 items-center gap-2 text-sm font-semibold text-blue-900 dark:text-white">
                          <PencilLine className="h-4 w-4 shrink-0" />
                          <span className="truncate">{t(language, 'activeSpecialQuantities')}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="rounded-full bg-white/80 px-2.5 py-1 text-xs font-bold text-blue-700 dark:bg-white dark:text-black">
                            {activeSpecialConditions.length}
                          </span>
                          <ChevronDown className={`h-4 w-4 shrink-0 text-blue-700 transition-transform duration-200 dark:text-white ${isSpecialQuantitiesExpanded ? 'rotate-180' : ''}`} />
                        </div>
                      </button>
                      ...
                      <div className="mt-3 space-y-2">
                        {activeSpecialConditions.map(customer => {
                          ...
                          return (
                            <div key={customer.id} className="rounded-[1.25rem] border border-blue-100/80 bg-white/70 p-3 shadow-[0_6px_18px_rgba(15,23,42,0.04)] dark:border-zinc-700 dark:bg-black/40">
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2">
                                    <p className="truncate text-sm font-bold text-slate-800">{customer.name}</p>
                                    <span className="shrink-0 rounded-full bg-blue-100 px-2 py-0.5 text-xs font-bold text-blue-700 dark:bg-white dark:text-black">{cond.quantity} L</span>
                                  </div>
                                  <div className="mt-1 flex items-start gap-1.5 text-xs text-slate-500">
                                    <CalendarIcon className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                                    <span className="leading-5 break-words">{dateText}</span>
                                ...
                                <div className="flex shrink-0 items-center gap-1 rounded-full bg-slate-100/80 p-1">
                                  <button
                                    ...
                                    className="rounded-full p-2 text-blue-600 transition-colors hover:bg-blue-100 dark:text-white dark:hover:bg-zinc-800"
                                    title={t(language, 'edit')}
                                  >
                                    <PencilLine className="h-3.5 w-3.5" />
                                  </button>
                    ...
                    allDisplayEntries.map((entry) => (
                      <tr key={entry.id} className="border-b border-slate-100 dark:border-slate-800/50 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="py-2 px-4 text-center whitespace-nowrap">
                          <span className={entry.morningQuantity > 0 ? "font-bold text-blue-600 dark:text-white" : "text-slate-400"}>
                            {entry.morningQuantity > 0 ? `${entry.morningQuantity} L` : '-'}
                          </span>
                        </td>
                        <td className="py-2 px-4 text-center whitespace-nowrap">
                          <span className={entry.eveningQuantity > 0 ? "font-bold text-indigo-600 dark:text-white" : "text-slate-400"}>
                            ...
                            {(entry as any).isVirtualSkipped ? (
                              ...
                              <>
                                <Button
                                  ...
                                  className="h-8 w-8 text-blue-600 rounded-lg hover:bg-blue-50 dark:text-white dark:hover:bg-zinc-800"
                          ...
                          <div className="flex justify-end gap-1">
                            {(entry as any).isVirtualSkipped ? (
                              ...
                              <>
                                <Button
                                  variant="ghost" size="icon"
                                  className="h-8 w-8 rounded-lg text-blue-600 hover:bg-blue-50 dark:text-white dark:hover:bg-zinc-800"
                                  onClick={() => openEditDialog(entry)}
                                >
                                  <SquarePen className="w-4 h-4" />
                                </Button>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              <input 
                type="checkbox" 
                id="autoGenRange"
                checked={autoGenIsRange} 
                onChange={(e) => {
                  setAutoGenIsRange(e.target.checked)
                  setAutoGenError(null)
                }}
                ...
                <>
                  <div className="flex-1 space-y-1">
                    ...
                    <input 
                      ...
                      onChange={e => {
                        setAutoGenStartDate(e.target.value)
                        setAutoGenError(null)
                      }}
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 text-sm font-medium text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-shadow"
                    />
                  </div>
                  <div className="flex-1 space-y-1">
                    ...
                    <input 
                      ...
                      onChange={e => {
                        setAutoGenEndDate(e.target.value)
                        setAutoGenError(null)
                      }}
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 text-sm font-medium text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-shadow"
                    />
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-white/10">
              <Button 
                type="button"
                variant="outline"
                className="rounded-xl px-5 border-slate-200 hover:bg-slate-100 text-slate-700 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            ...
            {autoGenError && (
              <div className="bg-amber-50/80 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300 p-3 rounded-xl text-sm font-medium flex items-start gap-2 border border-amber-200 dark:border-amber-500/20">
