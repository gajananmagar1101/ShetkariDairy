...
public class ScheduledTasks {
    ...
    public void autoGenerateDailyMilkEntries() {
        ...
        log.debug("Running scheduled task: Checking auto-entry schedules for date {} at {}", currentDate, currentTime);
        try {
            int count = 0;
            ...
            if (count > 0) {
                ...
                log.debug("Scheduled task completed. No milk entries auto-generated at {}", currentTime);
            }
        } catch (Exception e) {
    ...
    public void autoMarkDailyLabourAttendance() {
        LocalDate currentDate = LocalDate.now(AUTO_ENTRY_ZONE);
        String currentTime = LocalTime.now(AUTO_ENTRY_ZONE).format(TIME_FORMATTER);
        log.debug("Running scheduled task: Checking labour attendance schedules for date {} at {}", currentDate, currentTime);
        try {
            int count = 0;
            for (User user : userRepository.findAll()) {
            ...
            if (count > 0) {
                ...
                log.debug("Labour attendance scheduler completed. No attendance entries marked at {}", currentTime);
            }
        } catch (Exception e) {
