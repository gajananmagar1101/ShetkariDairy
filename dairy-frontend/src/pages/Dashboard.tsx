...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">{t(language, 'dashboardOverview')}</h1>
        <p className="text-slate-500 text-sm mt-1">{t(language, 'dashboardDesc')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.div
            ...
            className="p-5 rounded-3xl bg-white/60 border border-white/60 shadow-sm backdrop-blur-md"
          >
      ...
      <motion.div
        ...
        className="p-6 rounded-3xl bg-white/60 border border-white/60 shadow-sm backdrop-blur-md h-[400px]"
      >
        <h3 className="text-lg font-bold text-slate-800 mb-6">{t(language, 'weeklyTrends')}</h3>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data.weeklyTrends} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="milk" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorMilk)" />
          </AreaChart>
        </ResponsiveContainer>
      </motion.div>
    </div>
