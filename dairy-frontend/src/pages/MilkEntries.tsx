  ...
  const confirmAutoGenerate = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        const responseMessage = res.data.message as string | undefined
        if (count > 0) {
          setAutoGenError(null)
          ...
          setAutoGenError(
            language === 'mr'
              ? (autoGenIsRange
                  ? 'निवडलेल्या तारखांसाठी एंट्री आधीच आहे. डुप्लिकेट एंट्री तयार केली नाही.'
                  : `${autoGenStartDate} या तारखेची एंट्री आधीच आहे.`)
              : (responseMessage || 'Entries already exist. Skipped duplicate generation.')
          )
        }
      }
    } catch (error) {
      ...
      if (errorMsg.includes('Entries already exist')) {
        let mrError = 'या तारखांच्या नोंदी आधीच अस्तित्वात आहेत, त्यामुळे जनरेट करता येणार नाही.';
        if (errorMsg.includes('from') && errorMsg.includes('to')) {
          const parts = errorMsg.split('from ')[1].split(' to ');
          if (parts.length === 2) {
            const minDate = parts[0];
            const maxDate = parts[1].split('.')[0];
            mrError = `${minDate} ते ${maxDate} च्या दरम्यान नोंदी आधीच अस्तित्वात आहेत.`;
          }
        } else if (errorMsg.includes('for ')) {
          const datePart = errorMsg.split('for ')[1].split('.')[0];
          mrError = `${datePart} ची नोंद आधीच अस्तित्वात आहे.`;
        }
        setAutoGenError(language === 'mr' ? mrError : errorMsg);
      } else {
